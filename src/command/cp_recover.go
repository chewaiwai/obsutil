...
func (c *cpCommand) recoverTasks(fileUrls []string, metadata map[string]string, aclType obs.AclType, storageClassType obs.StorageClassType) error {
	...
	var mode cpMode = -1
	var totalAmounts int64 = 0
	var totalAmountsForProgress int64 = 0
	...
	for _, fileUrl := range fileUrls {
		assist.FindMatches(fileUrl, fileUrlPattern, action)
	}
...
func (c *cpCommand) recoverTask(mode cpMode, taskCtx map[string]string, metadata map[string]string,
	...
	if mode == um {
		...
		if isObsFolder(key) {
			...
			if !c.force && !confirm(fmt.Sprintf("Do you want download key [%s] in the bucket to [%s] ? Please input (y/n) to confirm:", key, bucket, fileUrl)) {
				return
			}
