...
func initChattri() command {
	...
	c.action = func() error {
		...
		checkParamFunc := func() bool {
			...
			if c.acl != "" {
				if _aclType, succeed := getAclType(c.acl); succeed {
					aclType = _aclType
				} else {
					return false
				}
		...
		prefixFunc := func(bucket, prefix string, batchFlag int) error {
			...
			if storageClassType != "" {
				if err := c.isNewVersionBucket(bucket); err != nil {
					printError(err)
					return assist.UnsupportedError
				}
	...
	c.help = func() {
		...
		printf("")
		printf("%2s%s", "", "-acl=xxx")
		printf("%4s%s", "", p.Sprintf("the ACL that can be specified for a bucket or objects, possible values are [private|public-read|public-read-write]"))
		printf("")
