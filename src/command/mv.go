...
func (c *transferCommand) scanObsDirAndDoAction(scan ScanObsDirCtx) (totalCount int64, totalBytesForProgress int64, totalObjects int64, hasListError error) {
	...
	for {
		...
		for _, content := range output.Contents {
			...
			if !isObsFolder(newRequest.src.key) {
...
func (c *transferCommand) cleanSource(request MoveRequestInput, _versionId string) (status int, requestId string, cleanObjectError error) {
	if deleteSrcSuccRequestId, err := c.deleteObject(request.src.bucket, request.src.key, request.src.versionId); err != nil {
		...
		warnLoggerMessage := fmt.Sprintf("%s, obs:%s/%s%s --> obs://%s/%s, warn message [%]",
			request.src.objectSizeStr, request.src.bucket, request.src.key, _versionId, request.dst.bucket, request.dst.key, deleteErrorMessage)
...
func (c *mvCommand) moveObjectWithMetaContext(request MoveRequestInput, srcMetaErr error, barCh progress.SingleBarChan, batchFlag int, fastFailed error) int {
	...
	if request.src.metaContext != nil {
		srcObjectSizeStr = assist.NormalizeBytes(request.src.metaContext.Size)
	}
	...
	var moveObjectError error = srcMetaErr
	if c.update {
...
func (c *mvCommand) recordMoveRate(request MoveRequestInput, _versionId string, moveObjectError error, cost int64, status int, requestId string, batchFlag int) int {
	if batchFlag >= 1 {
		if moveObjectError == nil {
			...
			status, code, message, requestId := c.checkAbort(moveObjectError, 401, 405)
			c.failedLogger.doRecord("%s, obs://%s/%s%s --> obs://%s/%s, cost [%d], status [%d], error code [%s], error message [%s], request id [%s]",
				request.src.objectSizeStr, request.src.bucket, request.src.key, _versionId, request.dst.bucket, request.dst.key, cost, status, code, message, requestId)
		}
	}
...
func initMv() command {
	...
	c.action = func() error {
		...
		//TODO
		moveRequestInput.isPosix = false
		moveRequestInput.canRename = false
		//TODO
		if c.jobs > 10 {
	...
	c.help = func() {
		...
		printf("")
		p.Printf("Syntax 1:")
		printf("%2s%s", "", "obsutil mv obs://srcbucket/key obs://dstbucket/[dest] [-dryRun] [-u] [-p=1] [-threshold=52428800] [-versionId=xxx] [-acl=xxx] [-sc=xxx] [-meta=aaa:bbb#ccc:ddd] [-ps=5242880] [-cpd=xxx] [-fr] [-o=xxx] [-config=xxx]")
		printf("")
		p.Printf("Syntax 2:")
		printf("%2s%s", "", "obsutil mv obs://srcbucket[/src_prefix] obs://dstbucket[/dest_prefix] -r [-dryRun] [-f] [-u] [-flat] [-j=1] [-p=1] [-threshold=52428800] [-acl=xxx] [-sc=xxx] [-meta=aaa:bbb#ccc:ddd] [-ps=5242880] [-include=*.xxx] [-exclude=*.xxx] [-timeRange=time1-time2] [-cpd=xxx] [-o=xxx] [-config=xxx]")
		printf("")
		...
		printf("")
		printf("%2s%s", "", "-acl=xxx")
		printf("%4s%s", "", p.Sprintf("the ACL of each object to be moved, possible values are [private|public-read|public-read-write]"))
		printf("")
		printf("%2s%s", "", "-sc=xxx")
